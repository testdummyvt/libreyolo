...
class LIBREYOLO9:
    ...
    Args:
        ...
        save_feature_maps: Feature map saving mode. Options:
            ...
            - List of layer names: Save only specified layers
        save_eigen_cam: If True, saves EigenCAM heatmap visualizations on each inference
        cam_method: CAM method for explain(). Options: "eigencam", "gradcam", etc.
    ...
    def __init__(
        ...
        if size not in ['t', 's', 'm', 'c']:
            raise ValueError(f"Invalid size: {size}. Must be one of: 't', 's', 'm', 'c'")
        ...
        # Initialize model
        self.model = LibreYOLO9Model(config=size, reg_max=reg_max, nb_classes=nb_classes)
    ...
    def export(self, output_path: str = None, input_size: int = 640, opset: int = 12) -> str:
        ...
        if output_path is None:
            if self.model_path and isinstance(self.model_path, str):
                ...
                output_path = f"libreyolo9{self.size}.onnx"

        print(f"Exporting LibreYOLO9 {self.size} to {output_path}...")
    ...
    def explain(
        ...
        Args:
            image: Input image
            method: CAM method(s) to use
            target_layer: Layer for CAM computation
            eigen_smooth: Apply SVD smoothing
            save: Save visualization to disk
