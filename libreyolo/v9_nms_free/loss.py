...
class YOLOv9NMSFreeLoss:
    """
    Combined loss for NMS-free YOLOv9 training.
    ...
    def __call__(
        self,
        preds_one2many: List[Tensor],
        preds_one2one: List[Tensor],
        targets: Tensor,
    ) -> Dict[str, Tensor]:
        """Compute combined loss for one-to-many and one-to-one branches."""
        ...
        box_value = box_loss.item() if isinstance(box_loss, Tensor) else box_loss
        dfl_value = dfl_loss.item() if isinstance(dfl_loss, Tensor) else dfl_loss
        cls_value = cls_loss.item() if isinstance(cls_loss, Tensor) else cls_loss

        num_fg = loss_many.get("num_fg", 0)
        if isinstance(num_fg, Tensor):
            num_fg = num_fg.item()
        num_fg += loss_one.get("num_fg", 0)
